...
String getDuration(String parseDate){
  if(parseDate==null||parseDate==""){
    return "";
  }
  DateTime temp = DateTime.parse(parseDate);
  if(DateTime.now().difference(temp).inDays>0){
    if(DateTime.now().difference(temp).inDays>30){
      return "${(DateTime.now().difference(temp).inDays/30).round()} month ago";
    }
    return "${DateTime.now().difference(temp).inDays} days ago";
  }else if(DateTime.now().difference(temp).inHours>0){
    return "${DateTime.now().difference(temp).inHours} hours ago";
  }else{
    return "${DateTime.now().difference(temp).inMinutes} minute ago";
  }
  return "";
}
